...
export const FeatureRow = ({ data, refFeatures }) => {
  return (
    <div className="w-full text-gray-700 dark:text-gray-300" ref={refFeatures}>
      ...
      <div className="flex flex-wrap mx-auto max-w-[1000px] items-center p-10 gap-y-10">
        <data.image
          ...
          color="#27be84"
          variant="Bulk"
          className={`mx-auto ${data.reversed || false ? "lg:order-last" : ""
...
export const SubFeatureRow = ({ data }) => {
  return (
    <div className="flex items-center gap-3 text-left">
      ...
      <div>
        <h3 className="text-gray-700 dark:text-gray-200 font-bold text-lg">{data.title}</h3>
        <p className="text-sm text-gray-700 dark:text-gray-400">{data.body}</p>
      </div>
    </div>
  );
};
